...
std::unordered_map <graph64, vector<double>> RandomGraphAnalysis::analyze(Graph& targetGraph, int randomGraphCount, int subgraphSize, const vector<double>& probs)
{
	...
	std::unordered_map<graph64, vector<double>> labelRelFreqsMap;
	//for (int i = 0; i < randomGraphCount; i++)
	for (int i = randomGraphCount; i--; )
	{
		//display status for every 100th graph
		if (i % 100 == 0)
		{
			std::cout << "Analyzing random graph " << 1000 - i << "..." << std::endl;
		}
